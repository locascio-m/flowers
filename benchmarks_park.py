...
#     # freq_std[i] = np.std(wr.freq_val*wr.ws)
...
for i in range(9):
    ...
    # im = ax.scatter(np.ma.masked_where(wr!=i+1,aep_flor),np.ma.masked_where(wr!=i+1,aep_flow),c=ws_std[i]*np.ma.masked_where(wr!=i+1,np.ones(200)),marker=markers[i],vmin=6.,vmax=9.,label='WR ' + str(i+1))
...
for i in range(9):
    im = ax0.scatter(np.ma.masked_where(wr!=i+1,aep_flor),np.ma.masked_where(wr!=i+1,aep_flow),marker=markers[i],label='WR ' + str(i+1))
# plt.colorbar(im,ax=ax0,label='Wind Rose Index')
# p = np.polyfit(aep_flor,aep_flow,1)
# error_avg = p[0]
xlim = ax0.get_xlim()
...
# Case 4
file_name = 'solutions/park' + str(4) + '.p'
var, aep_flowers, aep_floris, time_flowers, time_floris, layout_x, layout_y, wind_rose = pickle.load(open(file_name,'rb'))

fig, ax = plt.subplots()

aep_flow = np.array(aep_flowers) / 1e9
