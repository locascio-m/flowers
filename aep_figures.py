...
ax.set(xlabel='Conventional-Jensen Time [ms]',ylabel='FLOWERS Time [ms]')
ax.legend()
...
# p , _ = curve_fit(power_law,nt,time_flowers*1e3)
...
n_terms = np.linspace(10,180,endpoint=True)
...
ax2.set_title('Conventional-Jensen',fontweight='bold')
ax2.grid(linestyle=':')
ax3.set(xlim=[0,N], ylim=[0.975,1.025], xlabel='Iteration', ylabel='Normalized AEP')
ax3.set_title('Conventional-Gauss',fontweight='bold')
ax3.grid(linestyle=':')
...
# ax2.legend(['Jensen: $%.0f^\circ$' %(floris_resolution[0]),'Jensen: $%.0f^\circ$' %(floris_resolution[1]),'Jensen: $%.0f^\circ$' %(floris_resolution[2])],loc='lower left',fontsize=8)
...
for i in range(3):
    for j in range(3):
        if i == 0:
            ...
            if j == 0:
                model = park_aep[0]
                label = r'$\bf{Conventional}$' + '-' + r'$\bf{Jensen}$' + ' \n $\mathcal{D}$ = 360'
            elif j == 1:
                model = park_aep[1]
                label = '$\mathcal{D}$ = 72'
            elif j == 2:
            ...
            if j == 0:
                model = gauss_aep[0]
                label = r'$\bf{Conventional}$' + '-' + r'$\bf{Gauss}$' + ' \n $\mathcal{D}$ = 360'
            elif j == 1:
                model = gauss_aep[1]
                label = '$\mathcal{D}$ = 72'
            elif j == 2:
        ...
        axx = ax[j,i]
        axx.fill_between([-1,15],14,15,color='lightgray')
